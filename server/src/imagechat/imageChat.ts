 ...
 */
import { getGeminiApiKey } from "../settings/secrets.js";
import {
  extractInlineImage,
  NANO_BANANA_MODEL,
...
/*
 * The lab's version also exported `optimizeImagePrompt()`, a Claude-backed
 * rewrite of the user's words into a stronger image prompt. It is dropped here:
 * every avatar image path (portrait, character sheet, room placement) builds its
 * own prompt in avatar/look.ts, avatar/rooms.ts and avatar/characterSheet.ts and
 * calls generateChatImage() directly, so nothing in this repo ever called it —
 * and keeping it would pull in the lab's 955-line Claude client and its usage
 * accounting for a function with no callers.
 */
