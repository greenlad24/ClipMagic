import { z } from 'zod';
import { createEndpoint, Shots, PromoVideos, ZiteError } from 'zite-integrations-backend-sdk';
...
export default createEndpoint({
  authenticated: true,
  description: 'Generate media for a single shot. Screencast: segment-level retrieval + narrator-first pacing. B-Roll: tactical guard + Kinovi task (poll via pollBrollStatus).',
  inputSchema: z.object({ shotId: z.string() }),
  outputSchema: z.object({
    success:      z.boolean(),
    clipUrl:      z.string().optional(),
    kinoviTaskId: z.string().optional(),
    brollTrack:   z.enum(['pool', 'generated']).optional(),
  }),
  execute: async ({ input }) => {
    const shot = await Shots.findOne({ id: input.shotId });
    if (!shot) throw new ZiteError({ code: 'NOT_FOUND', message: 'Shot not found' });

    const tag = `[generateShot:${input.shotId}]`;
    ...
    try {
      ...
      if (shot.shotType === 'Screencast') {
        ...
        // Check if this beat follows the required tactical B-roll opening.
        ...
        if (availablePool.length === 0 && promoPool.length > 0) { availablePool = promoPool; }

        const result = await retrieveScreencast(client, caption, availablePool, beatDur, tag, ctx);
        if (result) {
          // Quality gate: weak match → prefer Talking Head
      ...
      if (shot.shotType === 'B-Roll') {
        ...
        if (!guard.allowed) {
          ...
          console.log(`${tag} ✅ Converted B-Roll → Talking Head (no promo match)`);
          return { success: true };
        }
        ...
        if (result) {
          ...
          return { success: true, kinoviTaskId: result.taskId, brollTrack: 'generated' as const };
        } else {
          await Shots.update({ id: input.shotId, record: { captureStatus: 'Error' } });
          return { success: false };
        }
